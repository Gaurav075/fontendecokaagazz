...
const BulkOrderForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!/^\d{10}$/.test(phone)) return "❌ Phone number should be exactly 10 digits";
    if (!quantity || quantity < 10) return "❌ Minimum quantity is 10";
  ...
  return (
    <section className="bg-[#faf9f7] py-16 px-6">
      <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-xl p-10 border border-neutral-200">
        ...
        {status && (
          <p className={`mt-6 text-center ${status.startsWith("✅") ? "text-green-600" : "text-red-600"}`}>
            {status}
          </p>
        )}
      </div>
