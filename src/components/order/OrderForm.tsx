...
const OrderForm: React.FC = () => {
  ...
  const [paymentData, setPaymentData] = useState<{ productId: string; quantity: number; price: number }[]>([]);
  ...
  useEffect(() => {
    ...
    if (storedCart) {
      try {
        ...
        setPaymentData(parsedCart.map(item => ({ 
  ...
  const displayRazorpay = async () => {
    ...
    try {
      ...
      const rzp = new (window as any).Razorpay({
        key: "rzp_live_HKAnIKAM9R6I8j", // Your Razorpay key
        ...
        handler: async function (response: any) {
          ...
          try {
            ...
            const verifyRes = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/payment/verify-order`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
            ...
            alert("Payment verification error. Please contact support with your payment details.");
          } finally {
            setLoading(false);
          }
        },
      ...
      alert(`Payment initialization failed: ${err instanceof Error ? err.message : 'Unknown error'}`);
      setLoading(false);
    }
  };
